...
axios.get(url)
  .then(response => {
    ...
    const data = response.data; // var with the data of tasks of each user
    for (const x of data) { // going through each data
      if (x.completed === true) { // if the tasks is completed
        if (dic[x.userId] !== undefined) { // if user is already in dic
          ...
          dic[x.userId] = status + 1;// here we set the value
        } else {
